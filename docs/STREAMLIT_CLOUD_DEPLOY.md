...
## Первый ввод реальных данных

Текущий MVP стартует с чистой PostgreSQL-БД без миграции SQLite, потому что реальных данных пока нет. Подробная инструкция: `docs/POSTGRESQL_REAL_DATA_START.md`.

Перед первым реальным вводом:

1. Заменить временные или раскрытые пароли.
2. Использовать отдельного PostgreSQL-пользователя приложения, а не owner/admin.
3. Проверить, что backup/restore у провайдера PostgreSQL понятен и доступен.
4. Ввести минимальный контрольный набор данных: единицы измерения, ингредиент, упаковку, продукт, первую закупку.
5. Перезапустить приложение и убедиться, что данные сохранились в PostgreSQL.
